...
def make_graph(pcd_points,pcd_colors,knn_edge_index,knn_edge_attr,visible_idx):
    ...
    points_norm = np.linalg.norm(pcd_points,axis=1) # convert xyz to (dist,uvw)
...
rects = [Rect(xmin=-9.4,xmax=-7.5,ymin=-6.4,ymax=-5.4),
         Rect(xmin=-8.4,xmax=-5.7,ymin=-5.5,ymax=-4.2),
         Rect(xmin=-6.6,xmax=-4,ymin=-4.2,ymax=-2.9),
         Rect(xmin=-4.6,xmax=-2.2,ymin=-3,ymax=-1.5),
         Rect(xmin=-2.7,xmax=-0.3,ymin=-1.5,ymax=-0.2),
         Rect(xmin=-0.7,xmax=1.7,ymin=-0.2,ymax=1.1),
         Rect(xmin=1.3,xmax=3.5,ymin=1.1,ymax=2.6),
         Rect(xmin=3.3,xmax=5.5,ymin=2.6,ymax=4.1),
         Rect(xmin=5.1,xmax=6.7,ymin=4.1,ymax=5.3)]
...
pcd_path = '../../../scratch/btuncay/cog/gnn_spatial_reasoning/datasets/terrains/terrains/scan_raw/combined_aligned.ply'
...
for vantage_idx, point in enumerate(zip(path,dirs)):
    ...
    out_path = f'../../../scratch/btuncay/cog/gnn_spatial_reasoning/datasets/processed/terrains/random_walks/rw_{process_idx}_{vantage_idx}.pt'
    point, viewdir = point
