...
path,dirs,_ = smooth_heading_walk(rng.choice(sampled_pts),rects,40,0.02,0.01,0.01,rng=rng)
#path = lazy_walk(rng.choice(sampled_pts),rects,100,0.1,0.9,0.05,rng=rng)
print(path,dirs)

pcd_path = '../../../scratch/btuncay/cog/gnn_spatial_reasoning/datasets/break_room/kicker/scan_raw/combined_aligned.ply'
...
for vantage_idx, point in enumerate(zip(path,dirs)):
    ...
    out_path = f'../../../scratch/btuncay/cog/gnn_spatial_reasoning/datasets/processed/break_room/random_walks/rw_{process_idx}_{vantage_idx}.pt'
    point, viewdir = point
