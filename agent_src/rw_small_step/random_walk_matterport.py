...
def smooth_heading_walk(start_xy, rects, n_steps=300,
    step=0.02, sigma_theta=0.05, sigma_step=0.01,
    # view-direction params
    pitch=-0.1,          # radians; negative looks slightly down (Z up)
    sigma_view=0.03,      # view yaw noise (smaller = steadier)
    view_pull=0.03,        # how strongly view yaw is pulled toward motion theta each step
    rng=None
    ):
    """
    Returns:
...
path,dirs,_ = smooth_heading_walk(rng.choice(sampled_pts),rects,40,0.08,0.05,0.01,rng=rng)
#path = lazy_walk(rng.choice(sampled_pts),rects,100,0.1,0.9,0.05,rng=rng)
print(path,dirs)

pcd_path = 'dataset_processed/anlieferung/delivery_area/scan_raw/combined_aligned.ply'
...
for vantage_idx, point in enumerate(zip(path,dirs)):
    ...
    out_path = f'dataset_processed/anlieferung/random_walks/rw_{process_idx}_{vantage_idx}.pt'
    point, viewdir = point
