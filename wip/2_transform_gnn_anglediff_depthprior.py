...
ALIAS = "0526_gnn_k2_l2_d32_anglediff_depth_rel"
EPOCHS = 40000
HIDDEN_DIM = 32
OUT_DIM = 4
GNN_LAYERS = 2 #2
...
##TODO:
# 1. split by scene, not within scene
# 2. exclude node ids from edge_hidden
...
def depth_dist(points: torch.Tensor) -> torch.Tensor:
    points = points.float()
    valid = torch.isfinite(points).all(dim=-1)
    return valid #returns coords of all points
...
def encode_observations_with_depth(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            depth_scales = outputs['mu_pcd_p'].detach().cpu()
    ...
    for (scene_name, walk_id), walk_observations in grouped.items():
        ...
        sequences.append(
            DepthWalkSequence(
                ...
                #depth_scales=torch.stack([obs.depth_scale for obs in walk_observations], dim=0),
                latents = torch.cat([torch.stack([obs.latent for obs in walk_observations],dim=0), torch.stack([obs.depth_scale for obs in walk_observations],dim=0)], dim=1),
...
class TransformGNN(nn.Module):
    ...
    def forward(self,nodes_in: torch.Tensor,edge_index: torch.Tensor,
        ...
        #edge_hidden = edge_attr # '_rel' model; adjust edge_head to use in=(latent_dim*1)
        pred_norm = self.edge_head(edge_hidden)
        #src_scale = pred_log_depth[src].exp().clamp_min(DEPTH_SCALE_EPS).unsqueeze(-1)
        #pred_metric = torch.cat([pred_norm[:, :3] * src_scale, pred_norm[:, 3:4]], dim=-1) #explicitly predicts depth scale
        pred_metric = torch.cat([pred_norm[:, :3], pred_norm[:, 3:4]], dim=-1)
        return pred_metric
...
def train() -> None:
    ...
    #train_sequences, val_sequences = split_sequences_by_scene(sequences)
