...
ALIAS = "0521_gnn_k2_l2_d64_anglediff_depthprior_relative"
EPOCHS = 40000
HIDDEN_DIM = 64
OUT_DIM = 4
GNN_LAYERS = 2 #2
...
# This variant distills the metric depth/point-cloud scale into a per-node
# scalar predicted from the latent. Translation is predicted in normalized
# depth units and rescaled by the source node depth prior.
# The transform head only sees relative latent/depth differences, which makes
# it harder to memorize absolute scene identity.
...
def point_depth_scale(points: torch.Tensor) -> torch.Tensor:
    points = points.float()
    valid = torch.isfinite(points).all(dim=-1)
    distances = points[valid].norm(dim=-1)
    distances = distances[torch.isfinite(distances) & (distances > DEPTH_SCALE_EPS)]
    if distances.numel() == 0:
        return points.new_tensor(1.0)
    return distances.median().clamp_min(DEPTH_SCALE_EPS)


def encode_observations_with_depth(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for idx in range(latents.size(0)):
                points = batch.pcd[batch.batch == idx, :3]
                depth_scales.append(point_depth_scale(points).detach().cpu())
            depth_scales = torch.stack(depth_scales, dim=0).float()
...
def encode_all_observations_with_depth(
    model: base.ImageGraphVAE,
    ...
    for root_dir in base.resolve_random_walk_roots(root_dirs):
        ...
        scene_observations = encode_observations_with_depth(
            model,
            loader,
...
def build_walk_sequences_with_depth(observations: Sequence[DepthObservation]) -> List[DepthWalkSequence]:
    grouped: Dict[Tuple[str, int], List[DepthObservation]] = {}
    ...
    sequences: List[DepthWalkSequence] = []
    for (scene_name, walk_id), walk_observations in grouped.items():
        ...
        sequences.append(
            DepthWalkSequence(
                ...
                depth_scales=torch.stack([obs.depth_scale for obs in walk_observations], dim=0).float(),
            )
        )
...
class TransformGNN(nn.Module):
    ...
    def regularize_latents(self, nodes_in: torch.Tensor) -> torch.Tensor:
        if not self.training:
            return nodes_in
        nodes = self.latent_dropout(nodes_in)
        if LATENT_NOISE_STD > 0:
            nodes = nodes + torch.randn_like(nodes) * LATENT_NOISE_STD
        return nodes
    ...
    def forward(
        ...
        src, dst = edge_index
        pred_log_depth = self.depth_head(nodes_in).squeeze(-1).clamp(LOG_DEPTH_MIN, LOG_DEPTH_MAX)
        nodes = self.regularize_latents(nodes_in)
        edge_delta = nodes[dst] - nodes[src]
        depth_delta = (pred_log_depth[dst] - pred_log_depth[src]).unsqueeze(-1)
        edge_input = torch.cat([edge_delta, depth_delta], dim=-1)
        pred_norm = self.edge_head(edge_input)
        src_scale = pred_log_depth[src].exp().clamp_min(DEPTH_SCALE_EPS).unsqueeze(-1)
        pred_metric = torch.cat([pred_norm[:, :3] * src_scale, pred_norm[:, 3:4]], dim=-1)
        if return_aux:
            return pred_metric, {
                "pred_norm": pred_norm,
                "pred_log_depth": pred_log_depth,
                "pred_depth_scale": pred_log_depth.exp().clamp_min(DEPTH_SCALE_EPS),
            }
        return pred_metric
...
def sequence_to_graph(sequence, k: int) -> Data:
    ...
    return Data(
        ...
        depth_scale=sequence.depth_scales.float().clamp_min(DEPTH_SCALE_EPS),
    )
...
def loss_terms(
    ...
    src = edge_index[0]
    src_scale = depth_scale[src].clamp_min(DEPTH_SCALE_EPS).unsqueeze(-1)
    valid = (
        torch.isfinite(pred).all(dim=-1)
        & torch.isfinite(target).all(dim=-1)
        & torch.isfinite(src_scale).all(dim=-1)
    )
    if not valid.any():
        zero = pred.sum() * 0.0
        return {
    ...
    edge_pred_norm = aux["pred_norm"][valid]
    edge_target_norm = edge_target[:, :3] / src_scale[valid]
    translation_loss = torch.sqrt(F.mse_loss(edge_pred[:, :3], edge_target[:, :3]))
    normalized_translation_loss = torch.sqrt(F.mse_loss(edge_pred_norm[:, :3], edge_target_norm))
    ...
    depth_valid = torch.isfinite(depth_scale) & (depth_scale > DEPTH_SCALE_EPS)
    if depth_valid.any():
        depth_scale_loss = F.smooth_l1_loss(
            aux["pred_log_depth"][depth_valid],
            depth_scale[depth_valid].clamp_min(DEPTH_SCALE_EPS).log().clamp(LOG_DEPTH_MIN, LOG_DEPTH_MAX),
        )
    else:
        depth_scale_loss = pred.sum() * 0.0
    loss = (
        TRANSLATION_WEIGHT * translation_loss.mean()
        + NORMALIZED_TRANSLATION_WEIGHT * normalized_translation_loss.mean()
        ...
        + DEPTH_SCALE_WEIGHT * depth_scale_loss.mean()
    )
    return {
        "loss": loss,
        "translation": translation_loss.mean(),
        "translation_norm": normalized_translation_loss.mean(),
        ...
        "depth_scale": depth_scale_loss.mean(),
    }
...
def run_epoch(
    ...
    with context:
        for batch in loader:
            ...
            pred, aux = model(batch.x.float(), batch.edge_index, return_aux=True)
...
def train() -> None:
    ...
    observations = encode_all_observations_with_depth(
        encoder,
        RANDOM_WALK_ROOTS,
        latent_branch=LATENT_BRANCH,
    )
    sequences = build_walk_sequences_with_depth(observations)
    ...
    for epoch in range(EPOCHS):
        ...
        row = {
            ...
            **prefix_keys("val", val_losses),
            ...
            "normalized_translation_weight": NORMALIZED_TRANSLATION_WEIGHT,
