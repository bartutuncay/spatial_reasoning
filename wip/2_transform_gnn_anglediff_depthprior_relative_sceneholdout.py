...
relative = load_module(
    "nips_transform_gnn_anglediff_depthprior_relative_base",
    Path(__file__).with_name("2_transform_gnn_anglediff_depthprior_relative.py"),
)

ALIAS = "0521_gnn_k2_l2_d64_anglediff_depthprior_relative_sceneholdout"
...
def split_sequences_by_scene(sequences: Sequence) -> Tuple[list, list]:
    ...
    if HELDOUT_SCENES:
        ...
        generator = torch.Generator().manual_seed(relative.SPLIT_SEED)
...
relative.ALIAS = ALIAS
relative.split_sequences = split_sequences_by_scene
