...
ALIAS = "0521_gnn_k1_l0_d64_anglediff_translation"
EPOCHS = 40000
HIDDEN_DIM = 64
TARGET_DIM = 4
OUT_DIM = 3
...
# This translation-only variant predicts only the frame-centric translation.
# During loss and rollout it reuses the GT yaw delta from the edge target.
...
def compose_transform_with_gt_yaw(
    pred_translation: torch.Tensor,
    target: torch.Tensor,
) -> torch.Tensor:
    return torch.cat([pred_translation[:, :3], target[:, 3:4]], dim=-1)
...
def loss_terms(
    ...
    angle_loss = pred.sum() * 0.0
    pred_transform = compose_transform_with_gt_yaw(pred, target)
