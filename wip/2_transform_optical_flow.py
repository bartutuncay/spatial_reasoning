...
RANDOM_WALK_ROOTS = [
    ...
    "../../../scratch/btuncay/cog/gnn_spatial_reasoning/datasets/processed/relief/rw_translation",
...
ALIAS = "0517_raft_flow_anglediff"
EPOCHS = 40000
HIDDEN_DIM = 64
OUT_DIM = 4
K_NEIGHBORS = 2 #4
FLOW_UPDATE_ITERS = 6
STORE_FEATURES_HALF = True
...
class ConvGRUCell(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        self.conv_q = nn.Conv2d(input_dim + hidden_dim, hidden_dim, kernel_size=3, padding=1)

    def forward(self, hidden: torch.Tensor, inputs: torch.Tensor) -> torch.Tensor:
        hx = torch.cat([hidden, inputs], dim=1)
        z = torch.sigmoid(self.conv_z(hx))
        r = torch.sigmoid(self.conv_r(hx))
        q = torch.tanh(self.conv_q(torch.cat([r * hidden, inputs], dim=1)))
        return (1.0 - z) * hidden + z * q


class RAFTStyleTransformEstimator(nn.Module):
    def __init__(self, in_channels: int, hidden_dim: int, out_dim: int, update_iters: int):
        super().__init__()
        ...
        motion_channels = hidden_dim + 6
        self.motion_encoder = nn.Sequential(
            nn.Conv2d(motion_channels, hidden_dim, kernel_size=3, padding=1),
            nn.GELU(),
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
            nn.GELU(),
        )
        self.update_block = ConvGRUCell(input_dim=hidden_dim * 2, hidden_dim=hidden_dim)
        self.delta_flow_head = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
            nn.GELU(),
    ...
    def correlation_flow(self, source: torch.Tensor, target: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        batch_size, channels, height, width = source.shape
        source_flat = F.normalize(source.flatten(2), dim=1, eps=1e-6)
        target_flat = F.normalize(target.flatten(2), dim=1, eps=1e-6)
        corr = torch.bmm(source_flat.transpose(1, 2), target_flat)
        attn = torch.softmax(corr, dim=-1)

        coords = self._coords(height, width, source.device, source.dtype)
        expected_coords = torch.matmul(attn, coords)
        source_coords = coords.unsqueeze(0).expand(batch_size, -1, -1)
        flow = (expected_coords - source_coords).transpose(1, 2).reshape(batch_size, 2, height, width)

        corr_strength = corr.max(dim=-1).values.reshape(batch_size, 1, height, width)
        entropy = -(attn * attn.clamp_min(1e-9).log()).sum(dim=-1)
        entropy = (entropy / math.log(float(height * width))).reshape(batch_size, 1, height, width)
        return flow, corr_strength, entropy

    def forward(self, node_features: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        if edge_index.numel() == 0:
            return node_features.new_empty((0, self.out_dim))

        src, dst = edge_index
        projected = self.feature_proj(node_features.float())
        source = projected[src]
        target = projected[dst]
        context = self.context_net(source)
        hidden = context
        flow = source.new_zeros(source.size(0), 2, source.size(2), source.size(3))

        corr_flow, corr_strength, corr_entropy = self.correlation_flow(source, target)
        feature_delta = target - source
        for _ in range(self.update_iters):
            residual_flow = corr_flow - flow
            motion_inputs = torch.cat(
                [feature_delta, flow, residual_flow, corr_strength, corr_entropy],
                dim=1,
            )
            motion = self.motion_encoder(motion_inputs)
            hidden = self.update_block(hidden, torch.cat([context, motion], dim=1))
            flow = flow + self.delta_flow_head(hidden)

        pooled_hidden = hidden.mean(dim=(-2, -1))
        pooled_flow = flow.mean(dim=(-2, -1))
        pooled_corr_flow = corr_flow.mean(dim=(-2, -1))
        pooled_confidence = corr_strength.mean(dim=(-2, -1))
        pooled_entropy = corr_entropy.mean(dim=(-2, -1))
        edge_hidden = torch.cat(
            [pooled_hidden, pooled_flow, pooled_corr_flow, pooled_confidence, pooled_entropy],
            dim=-1,
        )
...
def encode_feature_observations(
    encoder: nn.Module,
    loader,
    scene_name: str,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            features = extract_img_feature_map(encoder, img)
            features = F.normalize(features, dim=1, eps=1e-6).detach().cpu()
            if STORE_FEATURES_HALF:
...
def encode_all_feature_observations(
    encoder: nn.Module,
    ...
    for root_dir in base.resolve_random_walk_roots(root_dirs):
        ...
        scene_observations = encode_feature_observations(
            encoder,
            loader,
            scene_name=scene_name,
        )
        observations.extend(scene_observations)
        print(f"encoded {len(scene_observations)} feature observations from scene={scene_name}")

    observations.sort(key=lambda obs: (obs.scene_name, obs.walk_id, obs.step_id))
    return observations


def group_feature_observations_by_walk(
    observations: Sequence[FeatureObservation],
) -> Dict[Tuple[str, int], List[FeatureObservation]]:
    grouped: Dict[Tuple[str, int], List[FeatureObservation]] = {}
...
def build_feature_walk_sequences(observations: Sequence[FeatureObservation]) -> List[FeatureWalkSequence]:
    grouped = group_feature_observations_by_walk(observations)
    sequences: List[FeatureWalkSequence] = []
    ...
    for (scene_name, walk_id), walk_observations in grouped.items():
        ...
        sequences.append(
            FeatureWalkSequence(
                ...
                features=torch.stack([obs.feature for obs in walk_observations], dim=0),
...
def sequence_to_graph(sequence: FeatureWalkSequence, k: int) -> Data:
    edge_index = make_edge_index(sequence.features.size(0), k)
    return Data(
        x=sequence.features,
                ...
                writer.writerow(
                    {
                        ...
                        "steps": int(sequence.features.size(0)),
                        "edges": int(make_edge_index(sequence.features.size(0), K_NEIGHBORS).size(1)),
                    }
                )
...
def run_epoch(
    model: RAFTStyleTransformEstimator,
    loader: DataLoader,
...
def train() -> None:
    ...
    observations = encode_all_feature_observations(
        encoder,
        RANDOM_WALK_ROOTS,
    )
    del encoder
    ...
    sequences = build_feature_walk_sequences(observations)
    ...
    feature_hw = tuple(int(dim) for dim in train_graphs[0].x.shape[-2:])
    ...
    model = RAFTStyleTransformEstimator(
        ...
        out_dim=OUT_DIM,
        update_iters=FLOW_UPDATE_ITERS,
    ).to(device)
    ...
    print(
        ...
        f"{num_edges} pairwise edges, feature={in_channels}x{feature_hw[0]}x{feature_hw[1]}, "
        f"k={K_NEIGHBORS}, flow_iters={FLOW_UPDATE_ITERS}"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        row = {
            ...
            **prefix_keys("val", val_losses),
            ...
            "feature_height": feature_hw[0],
            "feature_width": feature_hw[1],
            "flow_update_iters": FLOW_UPDATE_ITERS,
